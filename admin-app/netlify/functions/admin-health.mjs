...
export function credentialStatus(now = new Date()) {
  const expiry = new Date(`${tokenExpiry}T00:00:00Z`)
  const daysLeft = Math.ceil((expiry.getTime() - now.getTime()) / 86400000)
  return {
    expiresAt: tokenExpiry,
    daysLeft,
    status: !process.env.VFY_GITHUB_CONTENT_TOKEN ? 'missing' : daysLeft <= 0 ? 'expired' : daysLeft <= 14 ? 'expiring' : 'ok',
  }
}
...
export default async function handler() {
  ...
  try {
    ...
    if(!result.ok)return Response.json({status:'unavailable',message:'Health monitoring is not connected yet.',credential})
    const data=await result.json()
    const run=data.workflow_runs?.[0]
    if(!run)return Response.json({status:'pending',message:'No completed health check yet.',credential})
    ...
    return Response.json({status,checkedAt,url:run.html_url,credential,
  ...
  }catch{return Response.json({status:'unavailable',message:'Could not reach the health monitor.',credential})}
}
